...
AppFont = 'Any 16'
sg.theme('DarkGrey5')
_VARS = {'cellCount': 20, 'gridSize': 700, 'canvas': False, 'window': False,
         'playerPos': [1*(700/20), 18*(700/20)], #gridSize/cellCount
         'cellMAP': False}
...
layout = [[sg.Canvas(size=(_VARS['gridSize'], _VARS['gridSize']),
          ...
          [sg.Exit('Quit', font=AppFont),
           ...
           sg.Button("Place Obstacle", font=AppFont)]]
...
while True:             # Event Loop
    ...
    if event == 'NewMaze':
        _VARS['playerPos'] = [1*cellSize, 18*cellSize]
        _VARS['cellMAP'] = makeMaze(_VARS['cellCount'], _VARS['cellCount'])
    ...
    if checkEvents(event) == 'Up':
        if int(_VARS['playerPos'][1] - 2*cellSize) >= 0: ##boundaries of the whole maze
            if _VARS['cellMAP'][yPos-2][xPos] != 1:
                _VARS['playerPos'][1] = _VARS['playerPos'][1] - cellSize
    elif checkEvents(event) == 'Down':
        if int(_VARS['playerPos'][1] + 2*cellSize) < _VARS['gridSize']-1:
            if _VARS['cellMAP'][yPos+2][xPos] != 1:
                _VARS['playerPos'][1] = _VARS['playerPos'][1] + cellSize
    elif checkEvents(event) == 'Left':
        if int(_VARS['playerPos'][0] - 2*cellSize) >= 0:
            if _VARS['cellMAP'][yPos][xPos-2] != 1:
                _VARS['playerPos'][0] = _VARS['playerPos'][0] - cellSize
    elif checkEvents(event) == 'Right':
        if int(_VARS['playerPos'][0] + 2*cellSize) < _VARS['gridSize']-1:
            if _VARS['cellMAP'][yPos][xPos+2] != 1:
                _VARS['playerPos'][0] = _VARS['playerPos'][0] + cellSize
    ...
    #drawCell(exitPos[0]*cellSize, exitPos[1]*cellSize, 'Black')
    for i in range (-1, 2):
        drawCell(_VARS['playerPos'][0]+i*cellSize, _VARS['playerPos'][1]-1*cellSize, 'TOMATO')
        drawCell(_VARS['playerPos'][0]+i*cellSize, _VARS['playerPos'][1], 'TOMATO')
        drawCell(_VARS['playerPos'][0]+i*cellSize, _VARS['playerPos'][1]+1*cellSize, 'TOMATO')
    placeCells()
